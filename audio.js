...
(function () {
  ...
  var SUPERSCRIPT_MAP = {
    '⁰':'0', '¹':'1', '²':'2', '³':'3', '⁴':'4', '⁵':'5', '⁶':'6', '⁷':'7', '⁸':'8', '⁹':'9',
    'ⁿ':'n', 'ᵐ':'m', 'ᵖ':'p', '⁻':' minus ', '⁺':' plus '
  };

  // Runs of Unicode superscript characters (math_notes writes exponents as
  // literal "2⁴³¹¹²⁶⁰⁹" or algebraic "xⁿ⁻¹", not <sup> tags) become spoken
  // exponents. Bare ² and ³ get the everyday "squared"/"cubed"; anything
  // else — multi-digit exponents like 2020, or algebraic ones like "n-1" —
  // becomes "to the power of ...".
  function expandSuperscripts(s) {
    return s.replace(/[⁰¹²³⁴⁵⁶⁷⁸⁹ⁿᵐᵖ⁻⁺]+/g, function (run) {
      if (run === '²') return ' squared';
      if (run === '³') return ' cubed';
      var body = run.replace(/[⁰¹²³⁴⁵⁶⁷⁸⁹ⁿᵐᵖ⁻⁺]/g, function (c) { return SUPERSCRIPT_MAP[c]; });
      return ' to the power of ' + body.replace(/\s+/g, ' ').trim();
    });
  }

  // √(...), √[...] or √token — captures a parenthesised/bracketed group or a
  // single bare number/letter run, since "√" alone gives no clue how far the
  // radicand extends. Re-run until no "√" remains: a radicand can itself
  // contain "√" ("√(6+√11)"), and replace() never rescans a captured group,
  // so a nested root would otherwise survive unconverted inside the outer
  // one. A leading space is added unconditionally and later collapsed by the
  // final \s+ pass, so "leg×√2" and "5√3" don't fuse into "the5the". A bare
  // "√" used as a label ("the named √ version") has nothing to bind to and
  // is simply named "square root".
  function expandSqrt(s) {
    var prev;
    do {
      prev = s;
      s = s.replace(/√(\([^)]*\)|\[[^\]]*\]|[A-Za-z0-9]+)/g, function (m, radicand) {
        return ' the square root of ' + radicand;
      });
    } while (s !== prev && s.indexOf('√') !== -1);
    return s.replace(/√/g, ' square root ');
  }

  var SUBSCRIPT_MAP = { '₀':'0', '₁':'1', '₂':'2', '₃':'3', '₄':'4', '₅':'5', '₆':'6', '₇':'7', '₈':'8', '₉':'9' };
  var SUBSCRIPT_LETTER_MAP = {
    'ᵢ':'i', 'ⱼ':'j', 'ₙ':'n', 'ₖ':'k', 'ₘ':'m', 'ₚ':'p', 'ₜ':'t', 'ᵣ':'r',
    '₋':' minus ', '₊':' plus '
  };

  // Mixed-number fractions written as whole + superscript + ⁄ + subscript
  // ("2²⁄₁₅" = 2 and 2/15) must be caught before the generic superscript/
  // subscript expanders below, or the ⁄ would be stranded between two
  // already-expanded phrases with no numeric meaning left.
  function expandUnicodeFractions(s) {
    return s.replace(/([0-9]*)([⁰¹²³⁴⁵⁶⁷⁸⁹]+)⁄([₀₁₂₃₄₅₆₇₈₉]+)/g, function (m, whole, num, den) {
      var n = num.replace(/[⁰¹²³⁴⁵⁶⁷⁸⁹]/g, function (c) { return SUPERSCRIPT_MAP[c]; });
      var d = den.replace(/[₀₁₂₃₄₅₆₇₈₉]/g, function (c) { return SUBSCRIPT_MAP[c]; });
      return (whole ? whole + ' and ' : '') + n + ' over ' + d;
    });
  }

  // Variable subscripts (a₁, pᵢ, xₙ, aₙ₋₁) -> "a sub 1", "p sub i", "x sub n",
  // "a sub n minus 1".
  function expandSubscripts(s) {
    return s.replace(/[₀₁₂₃₄₅₆₇₈₉ᵢⱼₙₖₘₚₜᵣ₋₊]+/g, function (run) {
      var out = '';
      for (var i = 0; i < run.length; i++) {
        var c = run[i];
        out += SUBSCRIPT_MAP[c] !== undefined ? SUBSCRIPT_MAP[c] : SUBSCRIPT_LETTER_MAP[c];
      }
  ...
  function normaliseText(s) {
    // Order matters: the mixed-number fraction pattern (whole+superscript+
    // ⁄+subscript) must be caught before the generic superscript/subscript
    // expanders consume its digits piecemeal.
    return expandSqrt(expandSuperscripts(expandSubscripts(expandUnicodeFractions(s))))
      // 14_Sequences_and_Recurrence.html writes some subscripts as a literal
      // underscore ("S_n", "T_n") instead of a Unicode subscript character.
      ...
      .replace(/\|/g, ' ')          // |A ∪ B| absolute-value bars are silent in speech
      .replace(/⌊\s*/g, ' the floor of ').replace(/\s*⌋/g, '')
      .replace(/⌈\s*/g, ' the ceiling of ').replace(/\s*⌉/g, '')
      .replace(/∠/g, ' angle ')
      .replace(/△/g, ' triangle ')
      .replace(/⟺/g, ' if and only if ')
      // "is ≤"/"is ≥" in source prose already supplies the verb ("is") —
      // don't double it into "is is less than...". A bare "≤"/"≥" (no
      // preceding "is") still needs its own verb.
      .replace(/\bis\s*≤/g, 'is less than or equal to')
      .replace(/\bis\s*≥/g, 'is greater than or equal to')
      .replace(/\bis\s*≠/g, 'is not equal to')
      ...
      .replace(/⁄/g, ' divided by ')  // bare fraction slash outside the mixed-number pattern ("2ⁿ ⁄ 2")
      .replace(/−/g, ' minus ')
      .replace(/³/g, ' cubed')
      ...
      .replace(/\bthe\s+the\b/gi, 'the')   // "(the √n test)" -> "(the the square root..." collapses back to one
      .replace(/\s+/g, ' ')
      .trim();
  }
  ...
  // index.html unit grid: <li><span class=un>01</span><span class=ut><a>Title</a>
  // <span class=ud>desc</span></span><span class=pc>19 problems</span></li> —
  // without this reader the number, title, description and count would run
  // together with no separators ("01Ratio and Proportionratios...19 problems").
  function readUnitListItem(li) {
    var un = txt(li.querySelector('.un'));
    var ut = li.querySelector('.ut');
    var ud = ut ? ut.querySelector('.ud') : null;
    var title = ut ? txt(ut).replace(ud ? txt(ud) : '', '').trim() : '';
    var pc = txt(li.querySelector('.pc'));
    var parts = [];
    if (un) parts.push('Unit ' + un + ':');
    if (title) parts.push(title + '.');
    if (ud) parts.push(txt(ud) + '.');
    if (pc) parts.push(pc + '.');
    return makeUnit(li, parts.join(' '));
  }
